...
namespace Microsoft.Azure.Zumo.Win8.CSharp.Test
{
    ...
    public class TypeExtensionsTests
    {
        ...
        {
            Assert.That (typeof(object).GetBaseTypesAndSelf().ToList(),
                   Is.EquivalentTo (new List<Type> { typeof(object) }));

            Assert.That (typeof(TestClassA).GetBaseTypesAndSelf().ToList(),
                   Is.EquivalentTo (new List<Type> { typeof(object), typeof(TestClassA) }));

            Assert.That (typeof(TestClassB).GetBaseTypesAndSelf().ToList(),
                   Is.EquivalentTo (new List<Type> {
                                        typeof(object),
                                        typeof(TestClassA),
                                        typeof(TestClassB) }));
        }
